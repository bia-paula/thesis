...
from dual_rnn import create_weighted_binary_crossentropy
...
def save_data(seqs, f_maps, label_encoding, vr, img_id, task, detects):
    try:
    ...
    try:
        ...
        pass
    np.savez_compressed(os.path.join( vr, "testing", task, img_id + '.npz'), seqs=seqs, detects=detects,
...
def beam_search(model, fmap_model, img_array, task, label_encoding_format, mem_size, vr, img_name,
    ...
    detects = np.zeros((mem_size, config.sequence_len, 1), dtype="float64")
    fmaps = {}
    ...
    for i in range(mem_size):
        detects[i, 0, :] = probs_detect[0, 0, :]
        detects[i, 1, :] = probs_detect[0, 0, :]
    ...
    for current_fix in range(2, config.sequence_len):
        if current_fix > 2:
            model = model_last
        # Predict next fp
        probs, probs_detect = model.predict([fmap_rnn[:, :current_fix], np.repeat([task_encoding], mem_size, axis=0)])
        ...
        detects[:, current_fix, :] = probs_detect[:, -1, :]
        detects[:, :, :] = detects[sub[1], :, :]
    ...
    detects = detects[sorted_inds]

    save_data(seqs, fmaps, task_encoding, vr, img_name, task, detects)
...
if __name__ == "__main__":
    ...
    path = ["/Volumes/DropSave/Tese/dataset/test_pairs_id.json",
            "/Volumes/DropSave/Tese/dataset/test_pairs_id_TA.json"]
    fp = open(path[0])
    data = json.load(fp)
    fp = open(path[1])
    ...
    image_dir = ["/Volumes/DropSave/Tese/dataset/resized_images",
                 "/Volumes/DropSave/Tese/dataset/coco_search18_images_TA"]
    ...
    rnn_dirs = glob("/Volumes/DropSave/Tese/trainedModels/dual/*/*.h5")
    rnn_dirs.sort()
    t = dict()
    dirs_len = len(rnn_dirs)

    dirs_w_test = glob("/Volumes/DropSave/Tese/trainedModels/dual/*/testing")
    ...
    for i_dir, rnn_path in enumerate(rnn_dirs):
        ...
        rnn = load_model(rnn_path, custom_objects={'weighted_binary_crossentropy': create_weighted_binary_crossentropy() })
        print(rnn.summary())
        ...
        for obs in data[i_0:]:
            ...
            if len(obs["bbox"]) == 0:
                ...
                aux_id = 0
            image_path = image_dir[aux_id] + "/" + class_name + "/" + image_id
